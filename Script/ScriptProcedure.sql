...
--LONG: THANH TOAN HOA DON HOC PHAN, THANH TOAN HOA DON CHUNG CHI, IN HOA DON CONG TY
--THANH TOÁN HÓA ĐƠN CHỨNG CHỈ
IF OBJECT_ID('THANHTOAN_HDCC', 'P') IS NOT NULL
DROP PROC THANHTOAN_HDCC
GO
CREATE PROC THANHTOAN_HDCC
	@TONGTIEN FLOAT,
	@THANHTOAN NVARCHAR(100),
	@MANV CHAR(4),
	@CMND CHAR(12)
AS	DECLARE @NGAYLAP VARCHAR	SET @NGAYLAP = CONVERT(VARCHAR, GETDATE(), 120)	--TẠO MÃ HÓA ĐƠN MỚI	DECLARE @MAHD CHAR(4), @COUNT INT = 0
	SELECT @COUNT = COUNT(@MAHD)
	FROM HD_CHUNGCHI
	SET @MAHD = 'HC' + CAST(@COUNT AS CHAR)
	WHILE EXISTS (SELECT * FROM HD_CHUNGCHI WHERE MAHD = @MAHD)
		BEGIN
			SET @COUNT +=1
			SET @MAHD = 'HC' + CAST(@COUNT AS CHAR)		END
	INSERT HD_CHUNGCHI VALUES (@MAHD, @CMND, @MANV, @TONGTIEN, @NGAYLAP, @THANHTOAN)
GO


--THANH TOÁN HÓA ĐƠN HỌC PHẦN
IF OBJECT_ID('THANHTOAN_HDHP', 'P') IS NOT NULL
DROP PROC THANHTOAN_HDHP
GO
CREATE PROC THANHTOAN_HDHP
	@TONGTIEN FLOAT,
	@THANHTOAN NVARCHAR(100),
	@MAHV CHAR(4),
	@MANV CHAR(4)
AS	DECLARE @NGAYLAP VARCHAR	SET @NGAYLAP = CONVERT(VARCHAR, GETDATE(), 120)	--TẠO MÃ HÓA ĐƠN MỚI	DECLARE @MAHD CHAR(4), @COUNT INT = 0
	SELECT @COUNT = COUNT(@MAHD)
	FROM HD_CHUNGCHI
	SET @MAHD = 'HH' + CAST(@COUNT AS CHAR)
	WHILE EXISTS (SELECT * FROM HOADONHOCPHAN WHERE MAHD = @MAHD)
		BEGIN
			SET @COUNT +=1
			SET @MAHD = 'HH' + CAST(@COUNT AS CHAR)		END
	INSERT HOADONHOCPHAN VALUES (@MAHD, @MANV, @MAHV, @TONGTIEN, @NGAYLAP, @THANHTOAN)
GO


--IN HÓA ĐƠN CÔNG TY
IF OBJECT_ID('INHOADONCONGTY', 'P') IS NOT NULL
DROP PROC INHOADONCONGTY
GO
CREATE PROC INHOADONCONGTY
	@MASOTHUE CHAR(4),
	@MAHV CHAR(4),
	@TENCONGTY NVARCHAR(100)
AS
	INSERT CONGTY VALUES (@MASOTHUE, @MAHV, @TENCONGTY)
GO